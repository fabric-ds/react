...
You must implement your own validation logic.

Validation is as easy as passing the property `invalid` with a `helpText` to
further explain the error. `helpText` can also be used as assistance label
before an error occurs and `invalid` is set to false.
