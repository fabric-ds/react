...
Selects can provide additional context with `hint` if the label and
placeholder aren't enough. You can force the hint text to always display by setting the `always` prop.
...
`invalid` is often paired with `hint` to provide feedback to the user about
the error.
